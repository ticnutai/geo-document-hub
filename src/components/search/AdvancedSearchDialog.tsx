    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[520px] max-h-[85vh] p-0 gap-0 overflow-hidden" dir="rtl">
        ...
        <Tabs value={tab} onValueChange={setTab} className="flex-1">
          ...
          {/* Zoning Tab */}
          <TabsContent value="zoning" className="mt-0 p-4 space-y-3">
            <div className="space-y-2">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="חיפוש מגרש או ייעוד..."
                  className="w-full rounded-lg border border-border bg-background pr-9 pl-3 py-2.5 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 transition-shadow"
                />
              </div>
              <select
                ...
                className="w-full rounded-lg border border-border bg-background px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-primary/50"
              >
                <option value="">כל הייעודים ({zoningData.length})</option>
                {zoningYeudTypes.map((y) => (
            ...
            <ScrollArea className="h-[260px]">
              <div className="space-y-1">
                {filteredZoning.map((z, i) => (
                  <button
                    key={i}
                    onClick={() => handleZoningSelect(z)}
                    className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-right hover:bg-accent/60 transition-all group"
                  >
                    <div className="h-3.5 w-3.5 rounded-full shrink-0 border border-border/40" style={{ backgroundColor: getYeudColor(z.yeudCode, z.yeudDesc) }} />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium truncate">מגרש {z.migrash} · {z.yeudDesc}</p>
                      <p className="text-[10px] text-muted-foreground truncate">{z.tabaYeud} · {z.helkaArea > 0 ? `${z.helkaArea.toLocaleString("he-IL")} מ״ר` : ""}</p>
                    </div>
                    <ChevronLeft className="h-3.5 w-3.5 text-muted-foreground/50 shrink-0" />
                  </button>
                ))}
                {zoningData.length === 0 && (
                  <div className="text-center py-8">
                    <Palette className="h-6 w-6 text-muted-foreground/30 mx-auto mb-2" />
                    <p className="text-xs text-muted-foreground">טוען ייעודי קרקע...</p>
                  </div>
                )}
              </div>
            </ScrollArea>
